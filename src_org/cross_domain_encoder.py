...
class TaskAdapter(nn.Module):
    ...
    def forward(self, x):
        ...
        return x * (1 + scale)  # 残差连接

class CrossDomainBertEncoder(nn.Module):
    """跨域BERT编码器：包含领域无关层和领域特异性层"""
    ...
    def __init__(self, args):
        ...
        # 添加架构配置参数
        self.agnostic_layers_num = getattr(args, 'agnostic_layers_num', 6)  # 默认6层
        self.specific_layers_num = 12 - self.agnostic_layers_num  # 剩余层数
        # 领域无关层 (BERT的前几层)
        self.domain_agnostic_layers = nn.ModuleList([
            copy.deepcopy(self.bert.encoder.layer[i]) for i in range(self.agnostic_layers_num)
        ])
        
        # 领域特异性层 (BERT的后几层，每个域一个)
        self.domain_specific_layers = nn.ModuleDict()
        self.domains = ['HuffPost', 'Amazon']  # 4个域
    ...
    def get_domain_from_dataset(self, text):
        """从数据中推断域信息 - 这里简化处理，实际应用中可能需要更复杂的逻辑"""
        # 在实际实现中，这个信息应该从数据加载器传入
        # 方法3: 从数据文件路径推断
